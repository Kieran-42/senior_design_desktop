...
class RearLeftLegController(Node):
    JOINT_NAMES = [
        'rear_left_base_hip_joint',
        'rear_left_hip_1_hip_2_joint',
        'rear_left_tibia_paw_joint',
    ]

    def __init__(self):
        super().__init__('rear_left_leg_controller_node')
        self.publisher = self.create_publisher(
            Float64MultiArray,
            '/rear_left_leg_controller/commands',
            10,
        )
        # Current target positions (radians)
        self.positions = [0.0, 0.0, 0.0]
        self.get_logger().info('Rear left leg controller ready.')
        self.print_help()

    def print_help(self):
        print('\n========== Rear Left Leg Controller ==========')
        print('Joints:')
